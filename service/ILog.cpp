...
void ILog::writeOut(const std::string &text) {
    std::cout << text;
}
...
std::string ILog::getCurrentTime(const std::string &mod) {
...
void ILog::writeLine(const std::string &text) {
    this->write(text + this->getNewLine());
}

void ILog::write(const std::string &text) {
    this->p_semaphore.acquire();
    this->writeOut(this->getCurrentTimeAndDate() + " || " + text);
    this->p_semaphore.release();
}

void ILog::writeError(const std::string &className, const std::string &functionName, const std::string &errorText) {
    this->writeLine(className + " :: " + functionName + " :: " + errorText);
}

void ILog::writeError(const std::string &className, const std::string &functionName, int errorCode) {
    this->writeError(className, functionName, std::to_string(errorCode));
}

void ILog::writeError(const std::string &className, const std::string &functionName, std::exception& e) {
